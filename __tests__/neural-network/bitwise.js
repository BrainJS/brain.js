...
function testBitwiseAdam(data, op) {
  const net = new NeuralNetwork();
  const res = net.train(data, { errorThresh: 0.003, learningRate: 0.05, praxis: 'adam' });
...
describe('bitwise functions sync training', () => {
  it('NOT function', () => {
    const not = [{input: [0], output: [1]},
               {input: [1], output: [0]}];
    testBitwise(not, 'not');
  });

  it('XOR function', () => {
    const xor = [{input: [0.001, 0.001], output: [0.001]},
