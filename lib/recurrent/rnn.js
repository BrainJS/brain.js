...
RNN.prototype = {
  ...
  getEquation: function(equation, inputMatrix, size, hiddenModel) {
    var copyToPreviousInput = equation.copyToPreviousInput.bind(equation);
  ...
  bindEquation: function() {
    ...
    for (i = 0, max = equation.states.length; i < max; i++) {
      ...
      if (!model.allMatrices.indexOf(state.left) < 0) {
        model.allMatrices.push(state.left);
      }
      if (!model.allMatrices.indexOf(state.right) < 0) {
        model.allMatrices.push(state.right);
      }
      if (!model.allMatrices.indexOf(state.into) < 0) {
  ...
  step: function() {
    ...
    for(var matrixIndex = 0; matrixIndex < matrixIndexes; matrixIndex++) {
      ...
      for(var i = 0, n = matrix.weights.length; i < n; i++) {
        ...
        if (isNaN(cache.weights[i]) || cache.weights[i] === Infinity) {
          console.log('die');
        }
        // gradient clip
        if (mdwi > clipval) {
        ...
        if (isNaN(matrix.weights[i]) || matrix.weights[i] === Infinity) {
          console.log('die');
        }
