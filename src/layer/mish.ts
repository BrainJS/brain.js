...
import { activate, measure } from '../activation/mish';
import { clear, makeKernel, release } from '../utilities/kernel';
...
export function compare2D(
  ...
  return measure(weights[this.thread.y][this.thread.x]);
}
...
export function compare3D(
  ...
  return measure(weights[this.thread.z][this.thread.y][this.thread.x]);
}
...
export function mish(inputLayer: ILayer, settings?: ILayerSettings): Mish {
  return new Mish(inputLayer, settings);
}
