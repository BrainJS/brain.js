...
export default class TimeStep {
  constructor(options = {}) {
    const defaults = TimeStep.defaults;
  ...
  bindEquation() {
    ...
    let equationConnection = model.equationConnections.length > 0
      ...
      // 0 index
    let output = this.getEquation(equation, equation.input(model.input.rows, model.input.columns), equationConnection[0], hiddenLayers[0]);
    outputs.push(output);
    // 1+ indices
  ...
  run(input = [], maxPredictionLength = 1, isSampleI = false, temperature = 1) {
    ...
    for (let i = 0; i < input.length; i++) {
      ...
      output.push(logProbabilities.weights[0]);
    }
    this.model.equations.length - 1;
   ...
   */
  train(data, options = {}) {
    options = Object.assign({}, TimeStep.trainDefaults, options);
