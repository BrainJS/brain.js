  ...
  _createClass(Relu, [{
    ...
    value: function setupKernels() {
      this.predictKernel = (0, _makeKernel2.default)(predict, {
        functions: [_relu.relu]
      });

      this.learnKernel = (0, _makeKernel2.default)(learn, {
        functions: [_relu.reluDerivative]
      });
    }
  }, {
    key: 'predict',
...
function predict(inputs) {
  return (0, _relu.relu)(inputs[this.thread.y][this.thread.x]);
}

function learn(weights, errors) {
  return (0, _relu.reluDerivative)(weights[this.thread.y][this.thread.x], errors[this.thread.y][this.thread.x]);
}
